...
class unet_3D_xy(object):
    ...
    def __init__(self, sess, param_set):
        ...
        self.loss_log       = param_set['loss_log']
        self.validatedata_dir = param_set['validatedata_dir']
        self.predict_label_dir = param_set['predict_label_dir']
    ...
    def unet_3D_model(self, inputI):
        ...
        with tf.device("/gpu:0"):
            ...
            print(inputI.shape)
            conv1_1 = conv3d(input=inputI, output_chn=32, kernel_size=3, stride=1, use_bias=False, name='conv1a')
            conv1_1_bn = tf.contrib.layers.batch_norm(conv1_1, decay=0.9, updates_collections=None, epsilon=1e-5, scale=True, is_training=phase_flag, scope="conv1_1_batch_norm")
            conv1_1_relu = tf.nn.relu(conv1_1_bn, name='conv1_1_relu')
            # conv1_2
            conv1_2 = conv3d(input=conv1_1_relu, output_chn=64, kernel_size=3, stride=1, use_bias=False, name='conv1b')
            conv1_2_bn = tf.contrib.layers.batch_norm(conv1_2, decay=0.9, updates_collections=None, epsilon=1e-5, scale=True, is_training=phase_flag, scope="conv1_2_batch_norm")
            conv1_2_relu = tf.nn.relu(conv1_2_bn, name='conv1_2_relu')

            pool1 = tf.layers.max_pooling3d(inputs=conv1_2_relu, pool_size=2, strides=2, name='pool1')
            #
            # conv2_1 = conv_bn_relu(input=pool1, output_chn=128, kernel_size=3, stride=1, use_bias=False, is_training=phase_flag, name='Conv2')
            conv2_1 = conv3d(input=pool1, output_chn=64, kernel_size=3, stride=1, use_bias=False, name='conv2a')
            conv2_2_bn = tf.contrib.layers.batch_norm(conv2_1, decay=0.9, updates_collections=None, epsilon=1e-5, scale=True, is_training=phase_flag, scope="conv2_1_batch_norm")
            conv2_2_relu = tf.nn.relu(conv2_2_bn, name='conv2_relu')

            conv2_2 = conv3d(input=conv2_2_relu, output_chn=128, kernel_size=3, stride=1, use_bias=False, name='conv2b')
            conv2_2_bn = tf.contrib.layers.batch_norm(conv2_2, decay=0.9, updates_collections=None, epsilon=1e-5, scale=True, is_training=phase_flag, scope="conv2_2_batch_norm")
            conv2_2_relu = tf.nn.relu(conv2_2_bn, name='conv2_relu')
            ...
            conv3_1 = conv3d(input=pool2, output_chn=128, kernel_size=3, stride=1, use_bias=False, name='conv3a')
            ...
            conv4_1 = conv3d(input=pool3, output_chn=256, kernel_size=3, stride=1, use_bias=False, name='conv4a')
        ...
        with tf.device("/gpu:1"):
            ...
            deconv1_3 = conv_bn_relu(input=deconv1_2, output_chn=256, kernel_size=3, stride=1, use_bias=False, is_training=phase_flag, name='deconv1_3')
            deconv2_1 = deconv_bn_relu(input=deconv1_3, output_chn=256, is_training=phase_flag, name='deconv2_1')
            #
            ...
            deconv2_3 = conv_bn_relu(input=deconv2_2, output_chn=128, kernel_size=3,stride=1, use_bias=False, is_training=phase_flag, name='deconv2_3')
            deconv3_1 = deconv_bn_relu(input=deconv2_3, output_chn=128, is_training=phase_flag, name='deconv3_1')
            #
            ...
            deconv3_3 = conv_bn_relu(input=deconv3_2, output_chn=64, kernel_size=3, stride=1, use_bias=False, is_training=phase_flag, name='deconv3_3')
            deconv4_1 = deconv_bn_relu(input=deconv3_3, output_chn=64, is_training=phase_flag, name='deconv4_1')
            #
            ...
            deconv4_3 = conv_bn_relu(input=deconv4_2, output_chn=32, kernel_size=3, stride=1, use_bias=False, is_training=phase_flag, name='deconv4_3')
            # predicted probability
            pred_prob = conv3d(input=deconv4_3, output_chn=self.output_chn, kernel_size=1, stride=1, use_bias=True, name='pred_prob')
    ...
    def train(self):
        ...
        img_clec, label_clec = load_data_pairs(self.traindata_dir, self.labeling_dir, self.resize_r, self.rename_map)
        # temporary file to save loss
        loss_log = open(self.loss_log, "w")
    ...
    def test(self):
        ...
        test_list = glob('{}/*.npy'.format(self.testdata_dir))
        test_list.sort()

        # test
        for k in range(0, len(test_list)):
            # get volume data
            vol_data = np.load(test_list[k])
            ...
            for i in range(len(self.rename_map)):
                composed_label[composed_orig == self.rename_map[i]] = self.rename_map[i]
            ...
            labeling_path = os.path.join(self.predict_label_dir, os.path.basename(test_list[k]))
            np.save(labeling_path, composed_label_resz)
    ...
    def test4crsv(self):
        ...
        # get file list of testing dataset
        test_list = glob('{}/*.npy'.format(self.validatedata_dir))
        test_list.sort()
        ...
        for k in range(0, len(test_list)):
            ...
            # evaluation
            gt_label = np.load(os.path.join(self.labeling_dir, os.path.basename(test_list[k])))
    ...
    def test_generate_map(self):
        ...
        for k in range(0, len(test_list)):
            ...
            for p in range(self.output_chn):
                ...
                c_map_path = os.path.join(self.predict_label_dir, os.path.basename(test_list[k]))
